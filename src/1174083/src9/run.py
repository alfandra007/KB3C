...
from keras.layers import Conv2D, Flatten, Dense, BatchNormalization, Reshape, concatenate, LeakyReLU, Lambda, \K, Activation, UpSampling2D, Dropout
...
def build_generator():
    ...
    input_z_noise = Input(shape=(latent_dims,)) #Input layer for vector z
    input_label = Input(shape=(num_classes,)) #Input layer for conditioning variable
...
def build_discriminator():
    ...
    label_shape = (6,) #Input conditioning variable shape
    image_input = Input(shape=input_shape) #Two input layers
    label_input = Input(shape=label_shape) #Two input layers
