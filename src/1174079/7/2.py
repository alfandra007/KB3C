# -*- coding: utf-8 -*-
"""
Created on Sun May  3 18:47:20 2020
...
with open('E:/hasy-data-labels.csv') as csvfile: #Membuka file hasy-data-labels.csv
    ...
    for row in csvreader: # Membuat looping pada variabel csvreader
        if i > 0: #Ketentuannya jika i lebih kecil daripada 0
            img = keras.preprocessing.image.img_to_array(pil_image.open("E:/HASYv2/" + row[0])) #Dibuat variabel img dengan isi keras untuk aktivasi neural network fungsi yang membaca data yang berada dalam folder HASYv2 dengan input nilai -1.0 dan 1.0
...
tensorboard = keras.callbacks.TensorBoard(log_dir='E:/KB/hasyv2/logs/mnist-style') #Membuat variabel tensorboard dengan isi lib keras 
...
for conv2d_count in [1, 2]: #Melakukan looping dengan ketentuan konvolusi 2 dimensi 1 2 
    for dense_size in [128, 256, 512, 1024, 2048]: #Menentukan ukuran besaran fixcel dari data atau konvert 1 fixcel mnjadi data yang berada pada codigan dibawah.
        for dropout in [0.0, 0.25, 0.50, 0.75]: #Membuat looping untuk memangkas masing-masing data dengan ketentuan 0 persen 25 persen 50 persen dan 75 persen.
            ...
            log_dir = 'E:/KB/hasyv2/logs/conv2d_%d-dense_%d-dropout_%.2f' % (conv2d_count, dense_size, dropout) #Melakukan log
