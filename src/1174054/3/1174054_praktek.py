...
dita = pd.read_csv('D:/Mata Kuliah/Tingkat 3/Semester 6/Kecerdasan Buatan/Chapter 3/drakor.csv',sep=';') #digunakan untuk memanggil file csv dan dipisahkan dengan ;
len(dita) #untuk mengetahui jumlah data
print(dita.juduldrakor) #digunakan untuk mengetahui kolom nama dari variabel dita
# In[2]: Soal2
import numpy as np #digunakan untuk mengimport library numpy
episode = np.sum(dita.jumlahepisode) # menggunakan fungsi sum bawaan dari numpy
...
plt.pie(slices, #untuk mengatur plot pie
  ...
  autopct='%1.1f%%')
plt.title('Pie Plot Dita') #memberikan judul pada pie plot
